...
new_test_data_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data/v2_bergermeer/")
...
class DummyTimeseriesDatasourceModel_Renier(object):
    def __init__(self, modeldb_path, resultnc_path):
        self.model_spatialite_filepath = modeldb_path
        self.resultnc_path = resultnc_path
        self.ds = NetcdfDataSource(resultnc_path, load_properties=False, ds="ignored")
...
class TestStatistics_Renier(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.stat = StatisticsTool(
            None,
            DummyTimeseriesDatasourceModel_Renier(
                os.path.join(new_test_data_dir, "v2_bergermeer.sqlite"),
                os.path.join(new_test_data_dir, "results_3di.nc"),
            ),
        )
    ...
    def test_files_exist(self):
        sqlite_path = os.path.join(new_test_data_dir, "v2_bergermeer.sqlite")
        sqlite1_path = os.path.join(new_test_data_dir, "results_3di.sqlite")
        self.assertTrue(os.path.exists(sqlite_path))
        self.assertTrue(os.path.exists(sqlite1_path))
    ...
    def test_flowlines(self):
        ...
        con_res = dbapi.connect(resultdb_path)
        con_res.row_factory = dbapi.Row
        flowline_cursor = con_res.execute("SELECT * FROM flowline_stats_view WHERE id=31878 ")
        flowline = flowline_cursor.fetchone()
        ...
        self.assertAlmostEqual(flowline["end_discharge"], 0.0355, places=3)

        self.assertAlmostEqual(flowline["max_velocity"], 0.006, places=3)
        self.assertAlmostEqual(flowline["end_velocity"], 0.0039, places=3)

        self.assertAlmostEqual(flowline["max_waterlevel_start"], -1.581, places=3)
        self.assertAlmostEqual(flowline["max_waterlevel_end"], -1.581, places=3)
        self.assertAlmostEqual(flowline["max_head_difference"], 9997.4, places=3)
