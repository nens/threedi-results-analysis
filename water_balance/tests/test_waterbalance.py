...
class WaterbalanceClassTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ts_datasource.spatialite_filepath = os.path.join(model_dir, 'vecht_s2.sqlite')
        items = [{
            'type': 'netcdf',
            'name': 'subgrid_map',
            'file_path': os.path.join(model_dir, 'vecht_s2_scenario', 'results', 'subgrid_map.nc')
        }]
        ...
        tr = QgsCoordinateTransform(
            QgsCoordinateReferenceSystem(28992, QgsCoordinateReferenceSystem.PostgisCrsId),
            QgsCoordinateReferenceSystem(4326, QgsCoordinateReferenceSystem.PostgisCrsId))
